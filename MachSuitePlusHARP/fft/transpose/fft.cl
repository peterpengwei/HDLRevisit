...
__kernel void 
__attribute__((task))
workload( __global TYPE * restrict work_x, __global TYPE * restrict work_y) {
...
loop5 : for(tid = 0; tid < 64; tid++){
            ...
            offset = lo * 66 + hi;
            data_y[0] = smem[0*8+offset];
            data_y[1] = smem[1*8+offset];
            data_y[2] = smem[2*8+offset];
            data_y[3] = smem[3*8+offset];
            data_y[4] = smem[4*8+offset];
            data_y[5] = smem[5*8+offset];
            data_y[6] = smem[6*8+offset];
            data_y[7] = smem[7*8+offset];
