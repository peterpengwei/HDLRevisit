...
void run_benchmark( void *vargs, cl_context& context, 
  ...
  //
  static unsigned *m1_buffer = (unsigned int*)clSVMAllocAltera(
                                  context, 0, sizeof(args->m1), 1024); 
  static unsigned *m2_buffer = (unsigned int*)clSVMAllocAltera(
                                  context, 0, sizeof(args->m2), 1024); 
  static unsigned *prod_buffer = (unsigned int*)clSVMAllocAltera(
                                  context, 0, sizeof(args->prod), 1024); 
  ...
  //
  int status;
  status = clSetKernelArgSVMPointerAltera(kernel, 0, (void*)m1_buffer);
  status |= clSetKernelArgSVMPointerAltera(kernel, 1, (void*)m2_buffer);
  status |= clSetKernelArgSVMPointerAltera(kernel, 2, (void*)prod_buffer);
  ...
  //
  memcpy(args->prod, prod_buffer, sizeof(args->prod));
}
