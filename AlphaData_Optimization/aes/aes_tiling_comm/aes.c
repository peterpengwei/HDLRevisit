...
#pragma HLS INTERFACE s_axilite port=return bundle=control
	...
	int compute_size = i == num_batches-1? tail_size:BUF_SIZE;
	memcpy(buf, data+i*BUF_SIZE, compute_size);
	// for (int j=0; j<compute_size; j+=16) {
    	//     aes256_encrypt_ecb(&ctx, local_key, buf+j);
	// }
	memcpy(data+i*BUF_SIZE, buf, compute_size);
    }
    return;
}
