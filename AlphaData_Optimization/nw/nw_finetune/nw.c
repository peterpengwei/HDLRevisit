...
#pragma HLS INTERFACE s_axilite port=return bundle=control
  ...
  #pragma HLS ARRAY_PARTITION variable=seqA_buf_z cyclic factor=32 dim=1
  ...
  #pragma HLS ARRAY_PARTITION variable=seqB_buf_z cyclic factor=32 dim=1
  ...
  #pragma HLS ARRAY_PARTITION variable=alignedA_buf_z cyclic factor=32 dim=1
  ...
  #pragma HLS ARRAY_PARTITION variable=alignedB_buf_z cyclic factor=32 dim=1
  ...
  for (i=0; i<num_batches+2; i++) {
    ...
    if (i % 3 == 0) {
      ...
      buffer_store(store_flag, alignedA+(i-2)*(ALEN+BLEN)*JOBS_PER_BATCH, alignedA_buf_z, alignedB+(i-2)*(ALEN+BLEN)*JOBS_PER_BATCH, alignedB_buf_z);
    } 
    else {
      buffer_load(load_flag, SEQA+i*ALEN*JOBS_PER_BATCH, seqA_buf_z, SEQB+i*BLEN*JOBS_PER_BATCH, seqB_buf_z);
